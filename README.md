...
- **RBAC 权限控制** — 基于角色的细粒度权限，21 个权限项
- **WAF 防火墙** — 实时检测 SQL 注入、XSS、命令注入、路径穿越、LDAP 注入、XXE、SSRF 等 7 类攻击
- **XSS 全局防护** — 后端 HTML 检测与转义 + 安全响应头（CSP/X-XSS-Protection），前端 React JSX 自动转义
...
- **16 个完整页面** — 无占位符，全部功能可用
- **响应式设计** — Ant Design + Tailwind CSS，桌面端完整适配，移动端侧边栏 Drawer 自适应
...
**计分引擎修复：**
- 将 `FirstBloodDetector` 首杀奖励接入实际计分路径 `ScoreCalculator`，此前首杀检测代码存在但未被调用

**前端修复：**
- `ChangePassword` 页面由英文改为中文，与全站保持一致
- 前端密码校验规则与后端对齐（最小 8 位 + 大小写 + 数字 + 特殊字符）
- 删除空 `Templates` 目录和遗留备份文件

**README 修正：**
- React 版本标识从 18 更正为 19
- 页面数量从 17 更正为 16
- RBAC 权限项从 16+ 更正为 21
- WAF 类型从 3 种更正为 7 种
- XSS 防护描述改为准确的后端 HTML 检测 + 安全响应头 + React JSX 自动转义
- 补充遗漏的 `PUT /api/v1/teams/:id` API 文档
