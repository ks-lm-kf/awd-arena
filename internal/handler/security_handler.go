...
var AlertManager = security.NewAlertManager(1000)
...
func GetAttackStats(c fiber.Ctx) error {
    ...
    db.Model(&model.FlagSubmission{}).
        ...
        Group("round").Order("round").Find(&roundCounts)

    total := correct + incorrect
    successRate := float64(0)
    if total > 0 {
        successRate = float64(correct) / float64(total) * 100
    }
